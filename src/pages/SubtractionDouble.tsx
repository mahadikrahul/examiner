...
export default function SubtractionDouble() {
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < TOTAL; i++) {
      // Double digit without borrowing:
      // a = 10 to 49. b is subtracted from a.
      // ones(b) <= ones(a), tens(b) <= tens(a)
      const a = randomInt(start, end);
      const onesA = a % 10;
      const tensA = Math.floor(a / 10);
      
      const onesB = randomInt(0, onesA); // No borrowing
      const tensB = randomInt(1, tensA); // Keep it double digit if possible
  ...
  return (
    <div className="worksheet-page">
      ...
      <h1>Double-Digit Subtraction ({start}–{end})</h1>
