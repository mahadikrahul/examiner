...
export default function Multiplication() {
  ...
  const handleApply = () => {
    let validStart = Math.max(0, Math.min(12, localStart));
    let validEnd = Math.max(0, Math.min(12, localEnd));
    ...
    <div className="worksheet-page">
      <div className="no-print worksheet-controls" style={{ display: 'flex', justifyContent: 'center', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', background: '#fff', padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
          ...
          <input type="number" min="0" max="12" value={localStart} onChange={(e) => setLocalStart(parseInt(e.target.value) || 0)} style={{ width: '60px', padding: '4px 8px', borderRadius: '4px', border: '1px solid #ccc' }} />
          <label style={{ fontWeight: 600, fontSize: '14px', marginLeft: '10px' }}>To:</label>
          <input type="number" min="0" max="12" value={localEnd} onChange={(e) => setLocalEnd(parseInt(e.target.value) || 10)} style={{ width: '60px', padding: '4px 8px', borderRadius: '4px', border: '1px solid #ccc' }} />
        </div>
      ...
      <h1>Multiplication ({start}–{end})</h1>
      <p className="instructions">Multiply the numbers and write the product below the line.</p>
