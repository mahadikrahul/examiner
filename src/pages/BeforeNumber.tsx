...
export default function BeforeNumber() {
  ...
  const generateQuestions = () => {
    ...
    for (let i = 0; i < TOTAL; i++) {
      // Avoid lowest number in the range (no before number for it)
      const num = randomInt(start + 1, end);
      newQuestions.push({ num });
    }
  ...
  return (
    <div className="worksheet-page">
      ...
      <h1>Before Number ({start}–{end})</h1>
      <p className="instructions">Write the number that comes before the given number.</p>
        ...
        {questions.map((q, index) => (
          <div key={index} className="question-col-3">
            <div className="question-row">
              ...
              <span className="worksheet-box">{q.num}</span>
            </div>
          </div>
        ))}
      </div>
